...
export function AttentionBanner({ events = [], locale = "de" }: { events?: BannerEvent[]; locale?: Locale }) {
  ...
  return (
    <div className="bg-black text-white text-[11px] font-bold tracking-[0.1em] uppercase">
      <div className="flex items-center justify-center h-9 px-4 max-w-[1400px] mx-auto gap-3">
        ...
        <p className="text-center truncate">{current.title} — {formatEventDate(current.date, locale)}</p>
        {events.length > 1 && (
