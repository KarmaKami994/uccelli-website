...
export function PartnerMarquee({ title, partners }: { title: string; partners: Partner[] }) {
  ...
  return (
    <section className="py-16 lg:py-20 border-y border-neutral-100 overflow-hidden" aria-labelledby="partner-marquee-title">
      ...
      <div className="partner-marquee relative overflow-hidden">
        <div className="partner-marquee-track flex w-max items-center gap-10 lg:gap-16 px-5">
          {repeated.map((partner, index) => {
            ...
            const duplicateClass = duplicate ? " partner-marquee-duplicate" : "";
            ...
            return partner.url ? (
              <a key={`${partner.name}-${index}`} href={partner.url} target="_blank" rel="noopener noreferrer" aria-hidden={duplicate || undefined} tabIndex={duplicate ? -1 : undefined} className={`h-20 min-w-[180px] px-6 flex items-center justify-center rounded-[12px] border border-neutral-100 bg-white hover:border-neutral-300 transition-colors${duplicateClass}`}>
                {content}
              </a>
            ) : (
              <div key={`${partner.name}-${index}`} aria-hidden={duplicate || undefined} className={`h-20 min-w-[180px] px-6 flex items-center justify-center rounded-[12px] border border-neutral-100 bg-white${duplicateClass}`}>
                {content}
              </div>
            );
          })}
      ...
      <style>{`
        .partner-marquee-track { animation: partner-marquee 32s linear infinite; }
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .partner-marquee-duplicate { display: none; }
        }
      `}</style>
    </section>
  );
