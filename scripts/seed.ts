/**
 * Seed script — fills an empty database with the initial Uccelli content.
 *
 *   npm run seed          (= payload run scripts/seed.ts)
 *
 * Idempotent: collections that already contain documents are skipped,
 * the Homepage/Navigation globals are only written when still empty.
 * German content is written to the default locale ("de"); English
 * translations can be added later in the admin panel (EN falls back
 * to DE until then).
 *
 * This replaces the former HTTP endpoints /api/seed and
 * /api/contact/seed, which were removed for security reasons.
 */
...
const projects = [
  { title: "LifeLab", slug: "lifelab", category: "bildung" as const, summary: "Lebenspraktische Kompetenzen für Jugendliche.", featured: true,
    sections: [
  ...
  { title: "Nightshift Music", slug: "nightshift-music", category: "gemeinschaft" as const, summary: "Konzertreihe für aufstrebende Artists.",
    sections: [
  ...
  { title: "Kleidersammelaktion", slug: "kleidersammelaktion", category: "sozial" as const, summary: "Sammlung und Verteilung von Kleidung an Bedürftige in Zürich.",
    sections: [
  ...
  { title: "Steuern & Versicherungs Schulung", slug: "steuern-versicherung", category: "bildung" as const, summary: "Workshops zu Steuern und Finanzplanung.",
    sections: [
  ...
  { title: "Meet & Greet", slug: "meet-and-greet", category: "sozial" as const, summary: "Regelmässige Treffen zum Austausch innerhalb der Community.",
    sections: [
  ...
  { title: "Uccelli Liga", slug: "uccelli-liga", category: "gemeinschaft" as const, summary: "Fussballturniere und sportliche Events.",
    sections: [
...
// NOTE: slug "datenschutz" (previously "datenschutz-wert") — the footer link
// and the /practice/datenschutz redirect point here.
const werte = [
  ...
  { title: "Datenschutz", slug: "datenschutz", body: "Der Schutz personenbezogener Daten ist für den Verein Uccelli von höchster Bedeutung. Wir behandeln alle Daten unserer Mitglieder und Partner mit grösster Sorgfalt und in Übereinstimmung mit dem Schweizer Datenschutzgesetz (nDSG).\n\nWir sammeln nur die Daten, die für unsere Vereinstätigkeiten notwendig sind, und geben keine Informationen an Dritte weiter." },
...
const navigationItems = [
  { label: "LifeLab", href: "/lifelab", children: [] as { label: string; href: string }[] },
  { label: "Programm", href: "/programm/projekte", children: [
  ...
  { label: "Netzwerk", href: "/netzwerk", children: [
  ...
  { label: "Kontakt", href: "/kontakt", children: [] as { label: string; href: string }[] },
];
...
async function run() {
  ...
  /** Seed a collection only when it's still empty (idempotent). */
  async function seedIfEmpty<T>(
    ...
    if (existing.docs.length > 0) {
      skipped.push(`${label} (${existing.totalDocs} vorhanden)`);
      return;
    }
    for (const item of items) {
      await payload.create({ collection, data: createFn(item) });
    }
    created.push(`${label}: ${items.length}`);
  }

  await seedIfEmpty("team-members", "Team", teamMembers, (m) => ({ name: m.name, role: m.role, bio: toRichText(m.bio), order: m.order }));
  await seedIfEmpty("faqs", "FAQ", faqs, (f) => ({ question: f.question, answer: toRichText(f.answer), order: f.order }));
  await seedIfEmpty("partners", "Partner", partners, (p) => ({ name: p.name, type: p.type, description: toRichText(p.description) }));
  await seedIfEmpty("posts", "Posts", posts, (p) => ({ title: p.title, slug: p.slug, date: p.date, summary: p.summary, body: toRichText(p.body) }));
  await seedIfEmpty("projects", "Projekte", projects, (p) => ({ title: p.title, slug: p.slug, category: p.category, summary: p.summary, body: toRichTextWithHeadings(p.sections), featured: p.featured ?? false }));
  await seedIfEmpty("networks", "Netzwerke", networks, (n) => ({ name: n.name, slug: n.slug, description: toRichText(n.description), order: n.order }));
  await seedIfEmpty("werte", "Werte", werte, (w) => ({ title: w.title, slug: w.slug, body: toRichText(w.body) }));
  await seedIfEmpty("courses", "Kurse", courses, (c) => ({ name: c.name, description: c.description, order: c.order }));
  await seedIfEmpty("pages", "Seiten", pages, (p) => ({ title: p.title, slug: p.slug, body: toRichText(p.body) }));
  await seedIfEmpty("events", "Events", events, (e) => ({ title: e.title, date: e.date, location: e.location, description: toRichText(e.description) }));

  // Globals: only write when still empty
  const nav = await payload.findGlobal({ slug: "navigation" });
  if (!nav.items || nav.items.length === 0) {
    await payload.updateGlobal({ slug: "navigation", data: { items: navigationItems } });
    created.push("Navigation (Global)");
  } else {
    skipped.push("Navigation (Global)");
  }

  const home = await payload.findGlobal({ slug: "homepage" });
  if (!home?.hero?.title) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await payload.updateGlobal({ slug: "homepage", data: homepage as any });
    created.push("Homepage (Global)");
  } else {
    skipped.push("Homepage (Global)");
  }

  console.log("\n── Seed abgeschlossen ──");
  if (created.length) console.log("Erstellt:  " + created.join(", "));
  if (skipped.length) console.log("Übersprungen (bereits befüllt): " + skipped.join(", "));
  if (!created.length) console.log("Alles bereits befüllt — nichts zu tun.");
  process.exit(0);
}
